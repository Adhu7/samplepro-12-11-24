/*
 * package dao; import bean.ArtistCategoryBean; import db.ArtDB; import
 * java.sql.*; import java.util.ArrayList; import java.util.List;
 * 
 * public class ArtistCategoryDAO {
 * 
 * public boolean addArtistCategory(ArtistCategoryBean category) { boolean
 * result = false; String query =
 * "INSERT INTO artist_category (categoryName) VALUES (?, ?)";
 * 
 ...
 * 
 * // Execute the insert operation int rowsAffected = ps.executeUpdate(); if
 * (rowsAffected > 0) { result = true; // Successfully inserted } } catch
 * (SQLException e) { e.printStackTrace(); }
 * 
 * return result; }
 * 
 * // Method to get all artist categories public List<ArtistCategoryBean>
 * getAllCategories() { List<ArtistCategoryBean> categories = new ArrayList<>();
 * String query = "SELECT * FROM artist_category";
 * 
 * try (Connection conn = ArtDB.getConnection(); PreparedStatement ps =
 * conn.prepareStatement(query); ResultSet rs = ps.executeQuery()) {
 * 
 * while (rs.next()) { ArtistCategoryBean category = new ArtistCategoryBean();
 * category.setCategoryId(rs.getInt("categoryId"));
 * category.setCategoryName(rs.getString("categoryName"));
 * categories.add(category); } } catch (SQLException e) { e.printStackTrace(); }
 * 
 * return categories; }
 * 
 * // Method to get a category by ID public ArtistCategoryBean
 * getCategoryById(int categoryId) { ArtistCategoryBean category = null; String
 * query = "SELECT * FROM artist_category WHERE categoryId = ?";
 * 
 * try (Connection conn = ArtDB.getConnection(); PreparedStatement ps =
 * conn.prepareStatement(query)) {
 * 
 ...
 * 
 * try (ResultSet rs = ps.executeQuery()) { if (rs.next()) { category = new
 * ArtistCategoryBean(); category.setCategoryId(rs.getInt("categoryId"));
 * category.setCategoryName(rs.getString("categoryName")); } } } catch
 * (SQLException e) { e.printStackTrace(); }
 * 
 * return category; }
 * 
 * // Method to update an artist category public boolean
 * updateCategory(ArtistCategoryBean category) { boolean result = false; String
 * query = "UPDATE artist_category SET categoryName = ? WHERE categoryId = ?";
 * 
 * try (Connection conn = ArtDB.getConnection(); PreparedStatement ps =
 * conn.prepareStatement(query)) {
 * 
 ...
 * return result; }
 * 
 * // Method to delete an artist category public boolean deleteCategory(int
 * categoryId) { boolean result = false; String query =
 * "DELETE FROM artist_category WHERE categoryId = ?";
 * 
 * try (Connection conn = ArtDB.getConnection(); PreparedStatement ps =
 * conn.prepareStatement(query)) {
 * 
 ...
 * 
 * int rowsAffected = ps.executeUpdate(); if (rowsAffected > 0) { result = true;
 * // Successfully deleted } } catch (SQLException e) { e.printStackTrace(); }
 * 
 * return result; } }
 */
